...
PARAMS = {
    ...
    # Bias for internal S1 mid-target (used in calculate_setpoints)
    "target_offset": 0.5,

    # Free-cooling / economizer: raise min fan flow when outdoor air can help (0 = off)
    "economizer_boost_flow": 0.0,
    "economizer_hot_margin": 0.8,      # only boost when max zone this close to S1 upper
}
...
class RBCModel:
    ...
    def calculate_setpoints(self, zone_temps, outdoor_temp, return_air_temp, occupancy,
        ...
        ebb = float(p.get("economizer_boost_flow", 0.0) or 0.0)
        if ebb > 0.0 and free_cooling_available and hot_margin < float(
            p.get("economizer_hot_margin", 0.8)
        ):
            flow = max(flow, ebb)
...
def _safe_rmtree(path: Path, attempts: int = 20, delay_s: float = 0.4) -> None:
    """Windows may keep eplus_out files open briefly after EnergyPlus returns."""
...
def run_simulation(params, output_dir: Path | None = None):
    """Run EnergyPlus with given params, return cost dict.

    output_dir: if set, write EnergyPlus outputs here (avoids Windows file locks
    when chaining runs). Default is OUT_DIR (eplus_out).
    """
    ...
    args = ["-d", str(out), "-w", str(EPW_FILE), "-r", str(IDF_FILE)]
